 ...
 * @param[in]    ImageIn    Pointer to the structure of the input image
 * @param[in]    ImageOut   Pointer to the structure of the ouput image
 * @param[in]    Img_tmp    Pointer to the temporary buffer used in the function, size must be 3*Number Pixels on a Line * 2 * sizeof(u16)
 * @return 
 *
 */
extern void arm_sobel_in_u8_out_f16_proc_f16_buff(const arm_image_gray_u8_t* ImageIn, arm_image_sobel_f16_t* ImageOut, arm_buffer_2_u16_t* Img_tmp);

/**
 * @brief        Sobel filter for grayscale data using a buffer to store intermediate values computing in q15
 * @param[in]    ImageIn    Pointer to the structure of the input image
 * @param[in]    ImageOut   Pointer to the structure of the ouput image
 * @param[in]    Img_tmp    Pointer to the temporary buffer used in the function, size must be 3*Number Pixels on a Line * 2 * sizeof(q15)
 * @return 
 *
 */
